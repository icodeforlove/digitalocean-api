...
Digitalocean.prototype._get = function(url, parameters, callback, required) {
	var params = extend(parameters, this.credentials); // Add credentials to parameters
	var getURL = API_URL + '/' + url + '?' + querystring.stringify(params); // Construct URL with parameters
		        ...
		        request.get(
					...
					function(error, response, body) {
						if (!error && body && !!body.status && body.status !== 'OK') {
							return reject(new Error(body.description || body.error_message));
						}
